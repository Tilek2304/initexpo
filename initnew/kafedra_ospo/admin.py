...
from .models import DepartmentOSPO, TeacherOSPO, ContactOSPO, AboutspiOSPO, AwardOSPO

class TeacherOSPOAdminForm(forms.ModelForm):
    bio = forms.CharField(widget=CKEditorWidget())
    class Meta:
        model = TeacherOSPO
        fields = '__all__'

class TeacherOSPOAdmin(admin.ModelAdmin):
    form = TeacherOSPOAdminForm
...
class AboutspiOSPOAdminForm(forms.ModelForm):
    content = forms.CharField(widget=CKEditorWidget())
    class Meta:
        model = AboutspiOSPO
        fields = '__all__'

class AboutspiOSPOAdmin(admin.ModelAdmin):
    form = AboutspiOSPOAdminForm

class ContactOSPOAdminForm(forms.ModelForm):
    content = forms.CharField(widget=CKEditorWidget())
    class Meta:
        model = ContactOSPO
        fields = '__all__'

class ContactOSPOAdmin(admin.ModelAdmin):
    form = ContactOSPOAdminForm

admin.site.register(TeacherOSPO, TeacherOSPOAdmin)
admin.site.register(ContactOSPO, ContactOSPOAdmin)
admin.site.register(AboutspiOSPO, AboutspiOSPOAdmin)
admin.site.register(AwardOSPO)  # Зарегистрировать модель AwardOSPO
